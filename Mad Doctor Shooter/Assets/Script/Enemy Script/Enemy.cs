...
public class Enemy : MonoBehaviour
{
    ...
    void SearchForPlayer()
    {
        ...
        if (Vector3.Distance(transform.position, playerTarget.position) > stoppingDistance)
        {
            ...
            enemyAnimation.PlayAnimation(TagManager.WALK_ANIMATON_NAME);
    ...
    void CheckIfAttackFinished()
    {
        if (Time.time > attackFinishedTimer)
            enemyAnimation.PlayAnimation(TagManager.IDLE_ANIMATON_NAME);
    }
    ...
    void Attack()
    {
        if(Time.time>attackTimer)
        {
            ...
            enemyAnimation.PlayAnimation(TagManager.ATTACK_ANIMATON_NAME);
        }
    }
}
